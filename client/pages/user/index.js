...
const UserIndex = () => {
  ...
  return (
    <UserRoute>
      {loading && <SyncOutlined spin className="d-flex jusitfy-content-center display-1 text-danger p-5"/>}
      <h1 className="jumbotron text-center square flux">{user && user.name}'s Courses</h1>
        ...
        <div key={course._id} className="media pt-2 pb-1">
            ...
            <div className="media-body pl-2">
              <div className="row">
                ...
                <Link href={`/user/course/${course.slug}`} className="pointer">
                    <a><PlayCircleOutlined className="h2text-primary"/></a>
                  </Link>
                </div>
              </div>
            </div>
